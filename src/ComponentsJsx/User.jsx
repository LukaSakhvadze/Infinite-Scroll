...
function User() {
  ...
  useEffect(() => {
    ...
    .then(response=>{
      ...
      setVisitedPosts([...visitedPosts,{prefix:response.data.prefix,name:response.data.name,lastName:response.data.lastName,id:response.data.id}])
    })
    axios.get(`http://sweeftdigital-intern.eu-central-1.elasticbeanstalk.com/user/${postId}/friends/1/20`)
    .then(response=>{
   ...
   return (
    <div className={styles.container}>
        ...
        {friendsPost.map((friendPost,index)=>(
          <div className={styles.box} onClick={()=>navigate(`/user?postId=${friendPost.id}`)} key={friendPost.id} ref={friendsPost.length===index+1?lastBox:undefined}>
            <div className={styles.picture}>
              <img src={friendPost.imageUrl} alt=""></img>
            </div>
            <div className={styles.subInfo}>
            <div className={styles.person}>{friendPost.prefix} {friendPost.name} {friendPost.lastName}</div>
